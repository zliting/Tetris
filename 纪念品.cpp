...
public:
	...
	{	
		...
		if(score>=10&&score<=12)Time_speed= 250;
		if(score>=12&&score<=14) Time_speed= 200;
		if(score>=14&&score<=16) Time_speed= 100;
	...
	void click()
	{
		if(kbhit())
		{
			...
			case 75:	ON_LEFT();break;
			case 77:	ON_RIGHT();break;
			case 80:	ON_DOWN();break;
	...
	int main()
	{
		...
		/*	int x=0,y=50;
			for(i=0;i<=26;i++)
			{
				for(j=1;j<=10;j++)
				{
					x=x+20;setcolor(WHITE);
				outtextxy(x,y,48+main_s[i][j]);
				}
				y=y+20;x=0;
			}*///
			click();
